...
describeAuthenticatedFlow('History Auth Flow (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!resolvedTenantId) {
      throw new Error(
        'Nenhum tenant disponível para execução do fluxo e2e de history',
      );
    }
    ...
    const createFarmResponse = await request(app.getHttpServer())
      ...
      .send({
        tenantId,
        name: `History E2E Farm ${uniqueSuffix}`,
        state: 'PR',
        city: 'Cascavel',
        totalAreaHa: 180,
      });
  ...
  it('should get season history for the farm', async () => {
    const response = await request(app.getHttpServer())
      .get(`${baseUrl}/farm/history/seasons/${farmId}`)
      .set('Authorization', `Bearer ${accessToken}`)
      .query({ tenantId });
  ...
  it('should get field history for the farm', async () => {
    const response = await request(app.getHttpServer())
      .get(`${baseUrl}/farm/history/fields/${farmId}`)
      .set('Authorization', `Bearer ${accessToken}`)
      .query({ tenantId });
  ...
  it('should get farm KPIs', async () => {
    const response = await request(app.getHttpServer())
      .get(`${baseUrl}/farm/history/kpis/${farmId}`)
      .set('Authorization', `Bearer ${accessToken}`)
      .query({ tenantId });
  ...
  it('should get crop comparison', async () => {
    const response = await request(app.getHttpServer())
      .get(`${baseUrl}/farm/history/comparison/${farmId}/soja`)
      .set('Authorization', `Bearer ${accessToken}`)
      .query({ tenantId });
