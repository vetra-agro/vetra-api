...
describeAuthenticatedFlow('Integrations Auth Flow (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!resolvedTenantId) {
      throw new Error(
        'Nenhum tenant disponível para execução do fluxo e2e de integrations',
      );
    }
  ...
  it('should list all integrations with status', async () => {
    const response = await request(app.getHttpServer())
  ...
  it('should save integration fields', async () => {
    const response = await request(app.getHttpServer())
      .put(`${baseUrl}/admin/integrations/weather`)
      .set('Authorization', `Bearer ${accessToken}`)
      .query({ tenantId })
      .send({ api_key: 'test-api-key-e2e' });
