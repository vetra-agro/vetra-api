...
describeAuthenticatedFlow('Audit Auth Flow (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!resolvedTenantId) {
      throw new Error(
        'Nenhum tenant disponível para execução do fluxo e2e de audit',
      );
    }
  ...
  it('should list audit logs filtered by module', async () => {
    const response = await request(app.getHttpServer())
      .get(`${baseUrl}/admin/audit/logs`)
      .set('Authorization', `Bearer ${accessToken}`)
      .query({ tenantId, module: 'farms', page: 1, limit: 10 });
